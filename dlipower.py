...
class PowerSwitch(object):
    ...
    def listScripts(self, scriptType):
        scripts = []
        url = self.geturl('script.htm')
        if not url:
            return None
        soup = BeautifulSoup(url,"html.parser")
        if scriptType == "all":
            root = soup.findAll('select',attrs={"name":"user_function"})
            if root:
                for temp in root[0].findAll('option'):
                    scripts.append(temp.text)
            return scripts
        
        if scriptType == "running":
            root = soup.findAll('tr',attrs={"class":"thread"})
            if root:
                for temp in root:
                    scriptName = temp.findAll('td',attrs={"class":'label'})
                    scriptId = temp.findAll('input',attrs={"name":"id"})
                    scripts.append([scriptName[0].text,scriptId[0]['value']])       
            return scripts
